...
app.directive('xListbox', function() {
    return {
        restrict: 'E',
        require: 'ngModel',
        scope: {},
        link: function(scope, element, attrs, ngModel) {
            // Hàm để cập nhật giao diện khi model thay đổi
            ngModel.$render = function() {
                // Ví dụ: cập nhật trạng thái các button dựa trên ngModel.$viewValue
                element.find('button').each(function() {
                    var btn = angular.element(this);
                    if (btn.attr('value') === ngModel.$viewValue) {
            ...
            // Lắng nghe sự kiện click từ các button để cập nhật giá trị model
            element.on('click', 'button', function() {
                scope.$apply(function() {
                    var newValue = angular.element(this).attr('value');
                    ngModel.$setViewValue(newValue);
                    // Nếu cần gọi ng-change thì Angular sẽ tự động kích hoạt nó khi model thay đổi
                }.bind(this));
            });
        }
    };
