    ...
    function() {
        ...
        theme.delegate = {
            ...
            throttle: function(limit, callback) {
                var waiting = !1;
                return function() {
                    waiting || (callback.apply(this, arguments), waiting = !0, setTimeout(function() {
                        waiting = !1
                    }, limit))
                }
            },
            ...
            trapFocus: function(options) {
                var eventsName = {
                    ...
                    focusableElements = elArray.filter(el => el.offsetParent !== null),
                    firstFocusable = focusableElements[0],
                    lastFocusable = focusableElements[focusableElements.length - 1];
                options.elementToFocus || (options.elementToFocus = options.container), options.container.setAttribute("tabindex", "-1"), options.elementToFocus.focus(), document.documentElement.off("focusin"), document.documentElement.on(eventsName.focusout, function() {
                    document.documentElement.off(eventsName.keydown)
                }), document.documentElement.on(eventsName.focusin, function(evt) {
                    evt.target !== options.container && evt.target !== lastFocusable && evt.target !== firstFocusable || document.documentElement.on(eventsName.keydown, function(evt2) {
                        _manageFocus(evt2)
                    })
                });

                function _manageFocus(evt) {
                    evt.keyCode === 9 && evt.target === lastFocusable && !evt.shiftKey && (evt.preventDefault(), firstFocusable.focus())
                }
            },
            ...
            lockMobileScrolling: function(namespace, element) {
                ...
                document.documentElement.classList.add("lock-scroll"), el.on("touchmove" + namespace, function() {
                    return !0
                })
            },
        ...
        theme.Currency = function() {
            ...
            function formatMoney(cents, format) {
                format || (format = theme.settings.moneyFormat), typeof cents == "string" && (cents = cents.replace(".", ""));
                var value = "",
                    placeholderRegex = /\{\{\s*(\w+)\s*\}\}/,
                    formatString = format || moneyFormat;

                function formatWithDelimiters(number, precision, thousands, decimal) {
                    if (precision = theme.utils.defaultTo(precision, 2), thousands = theme.utils.defaultTo(thousands, ","), decimal = theme.utils.defaultTo(decimal, "."), isNaN(number) || number == null) return 0;
            ...
            function getBaseUnit(variant) {
                if (variant && !(!variant.unit_price_measurement || !variant.unit_price_measurement.reference_value)) return variant.unit_price_measurement.reference_value === 1 ? variant.unit_price_measurement.reference_unit : variant.unit_price_measurement.reference_value + variant.unit_price_measurement.reference_unit
            }
            return {
                formatMoney,
                getBaseUnit
            ...
            function buildImagePath(string, widths) {
                ...
                if (widths) {
                    const imageUrls = [];
                    return widths.forEach(width => {
                        let url = `${string}?width=${width}`;
                        width === widths[widths.length - 1] ? url += ` ${width}w` : url += ` ${width}w,`, imageUrls.push(url)
                    }), imageUrls
                } else return [string]
            }
            return {
            ...
            _createInstance: function(container, constructor, scope) {
                ...
                if (constructor = constructor || this.constructors[type], !(typeof constructor > "u")) {
                    if (scope) {
            ...
            _onSectionLoad: function(evt, subSection, subSectionId) {
                ...
                if (section) {
                    ...
                    haveSubSections.length && this.loadSubSections(container), instance && typeof instance.onLoad == "function" && instance.onLoad(evt), setTimeout(function() {
            ...
            _onSectionUnload: function(evt) {
                this.instances = this.instances.filter(function(instance) {
                    ...
                    return isEventInstance && typeof instance.onUnload == "function" && instance.onUnload(evt), !isEventInstance
                })
            },
            ...
            register: function(type, constructor, scope) {
                ...
                scope && (sections = scope.querySelectorAll('[data-section-type="' + type + '"]')), sections.forEach(function(container) {
            ...
            return Variants.prototype = Object.assign({}, Variants.prototype, {
                _getCurrentOptions: function() {
                    var result = [];
                    return this.container.querySelectorAll(this.singleOptionSelector).forEach(el => {
                        var type = el.getAttribute("type");
                        type === "radio" || type === "checkbox" ? el.checked && result.push({
                            value: el.value,
                ...
                _getFullMatch(needsToBeAvailable) {
                    ...
                    return this.variants.find(variant => {
                        ...
                        return needsToBeAvailable ? isMatch && variant.available : isMatch
                    })
                },
                _getClosestAvailableMatch: function(lastSelectedOption) {
                    ...
                    return (lastSelectedOption && variants.filter(variant => currentlySelectedOptions.filter(({
                                                                                                                  value,
                                                                                                                  index
                                                                                                              }) => this._numberFromOptionKey(index) <= this._numberFromOptionKey(lastSelectedOption.index)).every(({
                                                                                                                                                                                                                        value,
                                                                                                                                                                                                                        index
                                                                                                                                                                                                                    }) => variant[index] === value) && variant.available)).reduce((bestMatch, variant) => {
                        if (bestMatch === null) return variant;
                        const bestMatchCount = this._getWeightedOptionMatchCount(bestMatch, lastSelectedOption);
                        return this._getWeightedOptionMatchCount(variant, lastSelectedOption) > bestMatchCount ? variant : bestMatch
                    }, null)
                },
                _getVariantFromOptions: function(lastSelectedOption) {
                    const availableFullMatch = this._getFullMatch(!0),
                        closestAvailableMatch = this._getClosestAvailableMatch(lastSelectedOption),
                        fullMatch = this._getFullMatch(!1);
                    return this.dynamicVariantsEnabled ? availableFullMatch || closestAvailableMatch || fullMatch || null : fullMatch || null
                },
                _updateInputState: function(variant, el) {
                    return input => {
                        ...
                        type === "radio" || type === "checkbox" ? (input.checked = variant[index] === value, input.checked && input.dispatchEvent(new Event("updateSwatch"))) : input.value = variant[index]
                    }
                },
                ...
                _updateImages: function(variant) {
                    variant.featured_image && this.container.dispatchEvent(new CustomEvent("variantImageChange", {
                        detail: {
                            variant
                        }
                    }))
                ...
                _updateMasterSelect: function(variant) {
                    ...
                    masterSelect && (masterSelect.value = variant.id, masterSelect.dispatchEvent(new Event("change", {
                        bubbles: !0
                    })))
                }
            }), Variants
        }(), window.vimeoApiReady = function() {
            theme.config.vimeoLoading = !0, checkIfVimeoIsReady().then(function() {
        ...
        theme.VimeoPlayer = function() {
            ...
            return VimeoPlayer.prototype = Object.assign({}, VimeoPlayer.prototype, {
                ...
                playerReady: function() {
                    this.iframe = this.el.querySelector("iframe"), this.iframe.setAttribute("tabindex", "-1"), this.options.loop === "false" && this.videoPlayer.setLoop(!1), this.options.style === "sound" ? this.videoPlayer.setVolume(1) : this.videoPlayer.setVolume(0), this.setAsLoaded(), new IntersectionObserver((entries, observer2) => {
                        entries.forEach(entry => {
                ...
                setAsLoaded: function() {
                    this.parent && (this.parent.classList.remove(classes.loading), this.parent.classList.add(classes.loaded), this.parent.classList.add(classes.interactable), Shopify && Shopify.designMode && window.AOS && AOS.refreshHard())
                },
            ...
            function YouTube(divId, options) {
                this.divId = divId, this.iframe = null, this.attemptedToPlay = !1, defaults.events = {
            ...
            return YouTube.prototype = Object.assign({}, YouTube.prototype, {
                ...
                onVideoPlayerReady: function(evt) {
                    this.iframe = document.getElementById(this.divId), this.iframe.setAttribute("tabindex", "-1"), this.options.style !== "sound" && evt.target.mute();
            ...
            function AjaxRenderer({
                ...
                this.sections = sections || [], this.cachedSections = [], this.onReplace = onReplace, this.debug = !!debug
            }
            return AjaxRenderer.prototype = Object.assign({}, AjaxRenderer.prototype, {
                renderPage: function(basePath, newParams, updateURLHash = !0) {
                    ...
                    return updateURLHash && this.updateURLHash(updatedParams), Promise.all(sectionRenders)
                },
                renderSectionFromCache: function(url, section) {
                    const cachedSection = this.cachedSections.find(url);
                    return this.log(`[AjaxRenderer] rendering from cache: url=${cachedSection.url}`), this.renderSection(cachedSection.html, section), Promise.resolve(section)
                },
                    ...
                    return this.log(`[AjaxRenderer] redering from fetch: url=${url}`), new Promise((resolve, reject) => {
                        fetch(url).then(response => response.text()).then(responseText => {
                            ...
                            this.cachedSections = [...this.cachedSections, {
                                html,
                                url
                            }], this.renderSection(html, section), resolve(section)
                ...
                renderSection: function(html, section) {
                    ...
                    } else section.nodeId.forEach(id => {
                ...
                getUpdatedParams: function(currentParams, newParams) {
                    ...
                    for (const [key, value] of clone.entries()) !newParams.getAll(key).includes(value) && !preservedParams.includes(key) && clone.delete(key);
                    for (const [key, value] of newParams.entries()) !clone.getAll(key).includes(value) && value !== "" && clone.append(key, value);
                    return clone
                },
            ...
            getCartProductMarkup: function() {
                ...
                return url = url.indexOf("?") === -1 ? url + "?view=ajax" : url + "&view=ajax", fetch(url, {
                    credentials: "same-origin",
                    method: "GET"
            ...
            _updateCart: function(params) {
                ...
                }).then(response => response.text()).then(cart => cart)
            },
            ...
            return CartForm.prototype = Object.assign({}, CartForm.prototype, {
                ...
                onSubmit: function(evt) {
                    if (this.submitBtn.classList.add(classes.btnLoading), document.documentElement.classList.contains("js-drawer-open") && this.cartItemsUpdated || document.documentElement.classList.contains("cart-open") && this.cartItemsUpdated) return this.submitBtn.classList.remove(classes.btnLoading), evt.preventDefault(), !1;
                    if (config.requiresTerms && !this.termsCheckbox.checked) return alert(theme.strings.cartTermsConfirmation), this.submitBtn.classList.remove(classes.btnLoading), evt.preventDefault(), !1
                },
                _parseProductHTML: function(text) {
                    const html = document.createElement("div");
                    return html.innerHTML = text, {
                ...
                cartMarkup: function(text) {
                    ...
                    this.updateCartDiscounts(markup.discounts), this.updateSavings(savings), count > 0 ? this.wrapper.classList.remove("is-empty") : this.wrapper.classList.add("is-empty"), this.updateCount(count), this.products.innerHTML = "", this.products.append(items), this.subtotal.innerHTML = theme.Currency.formatMoney(subtotal, theme.settings.moneyFormat), this.reInit(), window.AOS && AOS.refreshHard(), Shopify && Shopify.StorefrontExpressButtons && Shopify.StorefrontExpressButtons.initialize()
                },
                ...
                updateCount: function(count) {
                    ...
                    countEls.length && countEls.forEach(el => {
                        el.innerText = count
                    });
                ...
                if (!isTransitioning) {
                    ...
                    if (moduleId || (moduleId = el.dataset.controls), !!moduleId) {
                        if (!container) {
                            ...
                            multipleMatches.length > 0 && (container = el.parentNode.querySelector('[data-id="' + moduleId + '"]'))
                        }
                        if (!container) {
                            isTransitioning = !1;
                            return
            ...
            function setTransitionHeight(container, height, isOpen, isAutoHeight) {
                if (container.classList.remove(classes.hide), theme.utils.prepareTransition(container, function() {
            ...
            return Disclosure.prototype = Object.assign({}, Disclosure.prototype, {
                ...
                _toggleList: function() {
                    ...
                    this.cache.disclosureList.classList.toggle(classes.listVisible), this.cache.disclosureToggle.setAttribute("aria-expanded", !ariaExpanded)
                },
                destroy: function() {
            ...
            function Drawers(id, name) {
                ...
                }, this.drawer = document.querySelector("#" + id), this.isOpen = !1, this.drawer && this.init()
            }
            return Drawers.prototype = Object.assign({}, Drawers.prototype, {
                ...
                open: function(evt, returnFocusEl) {
                    evt && evt.preventDefault(), !this.isOpen && (evt && evt.stopPropagation ? (evt.stopPropagation(), evt.currentTarget.setAttribute("aria-expanded", "true"), this.activeSource = evt.currentTarget) : returnFocusEl && (returnFocusEl.setAttribute("aria-expanded", "true"), this.activeSource = returnFocusEl), theme.utils.prepareTransition(this.drawer, function() {
                ...
                bindEvents: function() {
                    window.on("click" + this.config.namespace, function(evt) {
                        this.close(evt)
                    }.bind(this)), window.on("keyup" + this.config.namespace, function(evt) {
            ...
            return Modal.prototype.init = function() {
                ...
                var externalCall = !1;
                this.modalIsOpen || (evt ? evt.preventDefault() : externalCall = !0, evt && evt.stopPropagation && (evt.stopPropagation(), this.activeSource = evt.currentTarget.setAttribute("aria-expanded", "true")), this.modalIsOpen && !externalCall && this.close(), this.modal.classList.add(this.config.openClass), document.documentElement.classList.add(...this.config.bodyOpenClass), this.isSolid && document.documentElement.classList.add(this.config.bodyOpenSolidClass), this.modalIsOpen = !0, setTimeout(() => {
                    theme.a11y.trapFocus({
                        container: this.modal,
                        elementToFocus: this.focusOnOpen,
                        namespace: "modal_focus"
                    })
                }, 100), document.dispatchEvent(new CustomEvent("modalOpen")), document.dispatchEvent(new CustomEvent("modalOpen." + this.id)), this.bindEvents())
        ...
        class ParallaxImage extends HTMLElement {
            constructor() {
                super(), this.parallaxImage = this.querySelector("[data-parallax-image]"), this.windowInnerHeight = window.innerHeight, this.isActive = !1, this.timeout = null, this.directionMap = {
                    right: 0,
                    top: 90,
                    left: 180,
                    bottom: 270
            ...
            getParallaxInfo() {
                ...
                let element = this.parallaxImage,
                    ...
                    movementPixels = Math.ceil(angle === "top" ? height * (parseFloat(movement) / 100) : width * (parseFloat(movement) / 100));
                angle = this.directionMap[angle] ?? parseFloat(angle), angle !== angle && (angle = 270), movementPixels !== movementPixels && (movementPixels = 100), angle %= 360, angle < 0 && (angle += 360);
                const toLeft = angle > 90 && angle < 270,
                    toTop = angle < 180;
                if (element.style[toLeft ? "left" : "right"] = 0, element.style[toTop ? "top" : "bottom"] = 0, angle % 90) {
                    const radians = angle * Math.PI / 180;
                    multipliers = [Math.cos(radians), Math.sin(radians) * -1]
                } else multipliers = this.directionMultipliers[angle];
                return multipliers[0] && (element.style.width = `calc(100% + ${movementPixels*Math.abs(multipliers[0])}px)`), multipliers[1] && (element.style.height = `calc(100% + ${movementPixels*Math.abs(multipliers[1])}px)`), {
                    element,
                    movementPixels,
                    multipliers,
                    top,
            ...
            init() {
                const {
                    ...
                    top,
                    height
                } = this.getParallaxInfo(), scrolledInContainer = this.windowInnerHeight - top, scrollArea = this.windowInnerHeight + height, progress = scrolledInContainer / scrollArea;
            ...
            scrollHandler() {
                this.isActive ? clearTimeout(this.timeout) : (this.isActive = !0, requestAnimationFrame(this.init.bind(this))), this.timeout = setTimeout(() => this.isActive = !1, 20)
            }
        }
        if (customElements.define("parallax-image", ParallaxImage), typeof window.noUiSlider > "u") throw new Error("theme.PriceRange is missing vendor noUiSlider: // =require vendor/nouislider.js");
        theme.PriceRange = function() {
            var defaultStep = 50000,
                selectors = {
            ...
            return PriceRange.prototype = Object.assign({}, PriceRange.prototype, {
                init: function() {
                    ...
                    return this.formEl = this.container.closest("form"),
                        ...
                        this.maxValue = parseFloat(this.container.dataset.maxValue) || this.maxRange, this.createPriceRange()
                },

                createPriceRange: function() {
                    this.sliderEl && this.sliderEl.noUiSlider && typeof this.sliderEl.noUiSlider.destroy == "function" && this.sliderEl.noUiSlider.destroy();
                    ...
                    return slider.on("update", values => {
            ...
            return ProductForm.prototype = Object.assign({}, ProductForm.prototype, {
                ...
                success: function(product) {
                    ...
                    errors && errors.remove(), theme.settings.cartType === "page" && (window.location = theme.routes.cartPage), this.form.dispatchEvent(new CustomEvent("ajaxProduct:added", {
                        detail: {
                            product,
                ...
                error: function(error) {
                    ...
                    errorDiv.classList.add("errors", "text-center"), typeof error.description == "object" ? errorDiv.textContent = error.message : errorDiv.textContent = error.description, this.form.append(errorDiv), this.form.dispatchEvent(new CustomEvent("ajaxProduct:error", {
                        detail: {
            ...
            function setupModelViewerListeners(model) {
                ...
                model.container.addEventListener("mediaVisible", function(event) {
                    xrButton.element.setAttribute("data-shopify-model3d-id", model.modelId), !theme.config.isTouch && event.detail.autoplayMedia && model.modelViewerUi.play()
            ...
            function QtySelector(el, options) {
                this.wrapper = el, this.plus = el.querySelector(selectors.plus), this.minus = el.querySelector(selectors.minus), this.input = el.querySelector(selectors.input), this.minValue = this.input.getAttribute("min") || 1;
                var defaults = {
                    namespace: null,
                    isCart: !1,
            ...
            return QtySelector.prototype = Object.assign({}, QtySelector.prototype, {
                init: function() {
                    this.plus.addEventListener("click", function() {
                        ...
                        var qty = this._getQty();
                        this._change(qty - 1)
                    }.bind(this)), this.input.addEventListener("change", function(evt) {
                ...
                _change: function(qty) {
                    qty <= this.minValue && (qty = this.minValue), this.input.value = qty, this.options.isCart && document.dispatchEvent(new CustomEvent("cart:quantity" + this.options.namespace, {
            ...
            products.forEach(product => {
                product.addEventListener("mouseover", productMouseover), product.addEventListener("focusin", productMouseover)
            });

            function productMouseover(evt) {
                var el = evt.currentTarget;
                if (!theme.config.bpSmall) {
                    if (el.removeEventListener("mouseover", productMouseover), el.removeEventListener("focusin", productMouseover), !el || !el.dataset.productId) return;
            ...
            url = url.replace("//", "/"), fetch(url).then(response => response.text()).then(text => {
                const html = document.createElement("div");
                html.innerHTML = text;
            ...
            function slideshow(el, args) {
                ...
                el.dataset.zoom && el.dataset.zoom === "true" && (this.slideshow.on("dragStart", () => {
                    ...
                    this.slideshow.slider.style.pointerEvents = "auto", this.slideshow.options.fade && (this.slideshow.slider.querySelector(".is-selected").style.pointerEvents = "auto")
                })), this.args.autoPlay) {
                    var wrapper = el.closest(selectors.wrapper);
                    this.pauseBtn = wrapper.querySelector(selectors.pauseButton), this.pauseBtn && this.pauseBtn.addEventListener("click", this._togglePause.bind(this))
                }
            ...
            return slideshow.prototype = Object.assign({}, slideshow.prototype, {
                ...
                slideChange: function(index) {
                    this.args.fade && this.currentSlide && (this.currentSlide.classList.add(classes.animateOut), this.currentSlide.addEventListener("transitionend", function() {
                ...
                initChildNav: function() {
                    this.childNavLinks[this.args.initialIndex].classList.add("is-active"), this.childNavLinks.forEach((link, i) => {
                        link.setAttribute("data-index", i), link.addEventListener("click", function(evt) {
                            evt.preventDefault(), this.goToSlide(this.getChildIndex(evt.currentTarget))
                        }.bind(this)), link.addEventListener("focus", function(evt) {
                            this.goToSlide(this.getChildIndex(evt.currentTarget))
                        }.bind(this)), link.addEventListener("keydown", function(evt) {
                ...
                childNavGoTo: function(index) {
                    ...
                    if (el.classList.add(classes.isActive), !!this.args.childNavScroller)
            ...
            return availability.prototype = Object.assign({}, availability.prototype, {
                init: function() {
                    this.container.on("variantChange" + this.namespace, this.setAvailability.bind(this)), this.setInitialAvailability()
                },
                ...
                setAvailability: function(evt) {
                    const {
                        ...
                        currentlySelectedValues,
                        variant
                    } = evt.detail, valuesToManage = this.createAvailableOptionsTree(this.variantsObject, currentlySelectedValues, variant, lastSelectedIndex, lastSelectedValue);
                    for (var [option, values] of Object.entries(valuesToManage)) this.manageOptionState(option, values, lastSelectedValue)
                },
                ...
                enableVariantOption: function(group, obj) {
                    ...
                    if (this.type === "dropdown") obj.soldOut ? group.querySelector('option[value="' + value + '"]').disabled = !0 : group.querySelector('option[value="' + value + '"]').disabled = !1;
                    else {
                        var buttonGroup = group.querySelector('.variant-input[data-value="' + value + '"]'),
                            input = buttonGroup.querySelector("input"),
                            label = buttonGroup.querySelector("label");
                        input.classList.remove(classes.disabled), label.classList.remove(classes.disabled), obj.soldOut && (input.classList.add(classes.disabled), label.classList.add(classes.disabled))
                    }
                },
            ...
            var youtubePlayer, vimeoPlayer, videoHolderId = "VideoHolder",
                selectors = {
            ...
            if (!youtubeTriggers.length && !vimeoTriggers.length && !mp4Triggers.length) return;
            var videoHolderDiv = document.getElementById(videoHolderId);
            youtubeTriggers.length && theme.LibraryLoader.load("youtubeSdk"), vimeoTriggers.length && theme.LibraryLoader.load("vimeo", window.vimeoApiReady);
            ...
            function closeVideoModal() {
                youtubePlayer && typeof youtubePlayer.destroy == "function" ? youtubePlayer.destroy() : vimeoPlayer && typeof vimeoPlayer.destroy == "function" ? vimeoPlayer.destroy() : emptyVideoHolder()
            }
        };
        class ToolTip extends HTMLElement {
            constructor() {
                super(), this.el = this, this.inner = this.querySelector("[data-tool-tip-inner]"), this.closeButton = this.querySelector("[data-tool-tip-close]"), this.toolTipContent = this.querySelector("[data-tool-tip-content]"), this.toolTipTitle = this.querySelector("[data-tool-tip-title]"), this.triggers = document.querySelectorAll("[data-tool-tip-trigger]"), document.addEventListener("tooltip:open", e => {
                    this._open(e.detail.context, e.detail.content)
                })
            }
            _open(context, insertedHtml) {
                this.toolTipContent.innerHTML = insertedHtml, context != "store-availability" && this.toolTipTitle.remove(), this._lockScrolling(), this.closeButton && this.closeButton.on("click.tooltip-close", () => {
                    this._close()
                ...
                }), this.el.dataset.toolTipOpen = !0, this.el.dataset.toolTip = context
            }
            _close() {
                this.toolTipContent.innerHTML = "", this.el.dataset.toolTipOpen = "false", this.el.dataset.toolTip = "", this._unlockScrolling(), this.closeButton.off("click.tooltip-close"), document.documentElement.off("click.tooltip-outerclick"), document.documentElement.off("keydown.tooltip-esc")
            }
            _lockScrolling() {
                theme.a11y.trapFocus({
                    container: this.el,
                    namespace: "tooltip_focus"
                }), theme.a11y.lockMobileScrolling(), document.documentElement.classList.add("modal-open")
            }
            _unlockScrolling() {
                theme.a11y.removeTrapFocus({
                    container: this.el,
                    namespace: "tooltip_focus"
                }), theme.a11y.unlockMobileScrolling(), document.documentElement.classList.remove("modal-open")
            }
        }
        ...
        class NewsletterReminder extends HTMLElement {
            ...
            init() {
                ...
                }), document.addEventListener(`modalOpen.${this.newsletterId}`, () => this.hide()), document.addEventListener(`modalClose.${this.newsletterId}`, () => this.show()), document.addEventListener("newsletter:openReminder", () => this.show(0)), this.closeBtn.addEventListener("click", () => {
                    this.hide(), Cookies.set(this.cookieName, "opened", {
                        path: "/",
                        expires: this.expiry
                    })
            ...
            show(time = this.secondsBeforeShow, forceOpen = !1) {
                sessionStorage.getItem("reminderAppeared") === "true" || setTimeout(() => {
        ...
        class ImageElement extends HTMLElement {
            ...
            init() {
                const handleIntersection = (entries, observer) => {
                    entries[0].isIntersecting && (this.removeAnimations(), observer.unobserve(this))
                };
            ...
            removeAnimations() {
                ...
                imageWrap && imageWrap.classList.add("loaded"), skrimWrap && skrimWrap.classList.add("loaded")
            }
        }
        ...
        class PredictiveSearch extends HTMLElement {
            constructor() {
                ...
                }), this.SearchModal && new MutationObserver(mutations => {
                    mutations.forEach(mutation => {
                        mutation.attributeName === "class" && mutation.target.className.indexOf("modal--is-active") > -1 && setTimeout(() => {
                            this.input.focus()
                        }, 100)
                    })
                }).observe(this.SearchModal, {
                    attributes: !0
            ...
            getSearchResults(searchTerm) {
                ...
                fetch(`${theme.routes.predictiveSearch}?${params}&section_id=search-results`).then(response => {
                    if (!response.ok) {
                    ...
                    this.predictiveSearchResults.innerHTML = resultsMarkup, this.open(), AOS.refreshHard()
                }).catch(error => {
                    throw this.close(), error
                })
            }
            ...
            close() {
                this.predictiveSearchResults.style.display = "none", this.predictiveSearchResults.innerHTML = "", this.classList.remove("is-active"), document.body.classList.remove("predictive-overflow-hidden"), document.dispatchEvent(new CustomEvent("predictive-search:close-all"))
            }
            debounce(fn, wait) {
                let t;
                return (...args) => {
        ...
        class RecipientForm extends HTMLElement {
            constructor() {
                super(), this.checkboxInput = this.querySelector(`#Recipient-Checkbox-${this.dataset.sectionId}`), this.emailInput = this.querySelector(`#Recipient-email-${this.dataset.sectionId}`), this.nameInput = this.querySelector(`#Recipient-name-${this.dataset.sectionId}`), this.messageInput = this.querySelector(`#Recipient-message-${this.dataset.sectionId}`), this.dateInput = this.querySelector(`#Recipient-send-on-${this.dataset.sectionId}`), this.addEventListener("change", () => this.onChange()), this.recipientFields = this.querySelector(".recipient-fields"), this.checkboxInput.addEventListener("change", () => {
            ...
            connectedCallback() {
                document.addEventListener("ajaxProduct:error", event => {
                    event.target.querySelector('[name="id"]').value === this.dataset.productVariantId && this.displayErrorMessage(event.detail.errorMessage)
                }), document.addEventListener("ajaxProduct:added", event => {
                    event.target.querySelector('[name="id"]').value === this.dataset.productVariantId && (this.clearInputFields(), this.clearErrorMessage())
                })
            }
            onChange() {
                this.checkboxInput.checked || (this.clearInputFields(), this.clearErrorMessage())
            }
            clearInputFields() {
                this.querySelectorAll(".field__input").forEach(el => el.value = "")
            }
            displayErrorMessage(body) {
                if (this.clearErrorMessage(), body) return Object.entries(body).forEach(([key, value]) => {
                    const inputElement = this[`${key}Input`];
                    inputElement && (inputElement.setAttribute("aria-invalid", !0), inputElement.classList.add("field__input--error"))
                })
            }
            clearErrorMessage() {
                this.querySelectorAll(".field__input").forEach(inputElement => {
                    inputElement.setAttribute("aria-invalid", !1), inputElement.removeAttribute("aria-describedby"), inputElement.classList.remove("field__input--error")
                })
            }
        }
        customElements.define("recipient-form", RecipientForm), theme.announcementBar = function() {
            ...
            function init() {
                bar = document.getElementById("AnnouncementSlider"), bar && (unload(), bar.dataset.blockCount !== 1 && ((theme.config.bpSmall || bar.dataset.compact === "true") && initSlider(), document.addEventListener("matchSmall", function() {
                    unload(), initSlider()
            ...
            function onBlockSelect(id) {
                ...
                flickity && typeof flickity.pause == "function" && (flickity.goToSlide(index), flickity.pause())
            }
            ...
            function CartDrawer() {
                this.form = document.querySelector(selectors.form), this.drawer = new theme.Drawers("CartDrawer", "cart"), this.init()
            }
            return CartDrawer.prototype = Object.assign({}, CartDrawer.prototype, {
                init: function() {
                    this.cartForm = new theme.CartForm(this.form), this.cartForm.buildCart(), document.addEventListener("ajaxProduct:added", function(evt) {
                ...
                open: function() {
                    this.drawer.open(), theme.a11y.trapFocus({
                        container: this.form,
                ...
                close: function() {
                    this.drawer.close(), theme.a11y.removeTrapFocus({
                        container: this.form,
                        namespace: "cartdrawer_focus"
                    })
                }
            ...
            function menuDetailsHandler() {
                document.querySelectorAll(selectors.navDetails).forEach(navDetail => {
                    const summary = navDetail.querySelector("summary");
                    document.addEventListener("click", evt => {
                        navDetail.hasAttribute("open") && !navDetail.contains(evt.target) ? (navDetail.removeAttribute("open"), summary.setAttribute("aria-expanded", "false")) : navDetail.hasAttribute("open") ? summary.setAttribute("aria-expanded", "false") : summary.setAttribute("aria-expanded", "true")
                    })
                })
            }
            ...
            function stickyHeader() {
                config.lastScroll = 0;
                var wrapWith = document.createElement("div");
                wrapWith.id = config.stickyHeaderWrapper, theme.utils.wrap(siteHeader, wrapWith), stickyHeaderInitialPosition(siteHeader), stickyHeaderHeight(), window.on("resize" + config.namespace, theme.utils.debounce(50, stickyHeaderHeight)), window.on("scroll" + config.namespace, theme.utils.throttle(20, stickyHeaderScroll)), Shopify && Shopify.designMode && setTimeout(function() {
                    stickyHeaderHeight()
                }, 250)
            }

            function stickyHeaderInitialPosition(header) {
                const parentNextSibling = header.closest(".shopify-section-group-header-group").nextElementSibling;
            ...
            function stickyHeaderHeight() {
                if (config.stickyEnabled) {
                    ...
                    var stickyHeader2 = document.querySelector("#" + config.stickyHeaderWrapper);
            ...
            function scrollHandler() {
                if (window.scrollY > 250) {
                    ...
                    config.stickyActive = !0, siteHeader.classList.add(config.stickyClass), config.wrapperOverlayed && wrapper.classList.remove(config.overlayedClass), setTimeout(function() {
            ...
            function searchDrawer() {
                document.querySelectorAll(selectors.searchBtn).forEach(btn => {
                    btn.addEventListener("click", function(evt) {
                        evt.preventDefault(), evt.stopPropagation(), document.dispatchEvent(new CustomEvent("predictive-search:open", {
                            detail: {
                                context: "header"
                            },
                            bubbles: !0
            ...
            return items.forEach(product => {
                ...
                let priceMarkup = "",
                    vendorMarkup = "";
                theme.settings.predictiveSearchPrice && (priceMarkup = `<div class="grid-product__price">${theme.strings.productFrom}${theme.Currency.formatMoney(product.price_min,theme.moneyFormat)}</div>`), theme.settings.predictiveSearchVendor && (vendorMarkup = `<div class="grid-product__vendor">${product.vendor}</div>`);
                var markup = `
      ...
      `;
            ...
            var size = theme.settings.productImageSize,
                output = "";
            if (size === "natural") {
                const clonedMarkup = document.getElementById("naturalImageMarkup").content.cloneNode(!0),
                    imageEl = clonedMarkup.querySelector("img"),
                    imageWrapperEl = clonedMarkup.querySelector(".image-wrap");
                imageWrapperEl.style.paddingBottom = `${product.image_aspect_ratio}%`, addImageProperties(imageEl, imageSizes);
                const imageDiv = document.createElement("div");
                imageDiv.appendChild(clonedMarkup), output = imageDiv.innerHTML
            } else {
                const clonedMarkup = document.getElementById("fixedRatioImageMarkup").content.cloneNode(!0),
                    imageEl = clonedMarkup.querySelector("img");
                clonedMarkup.querySelector(".grid__image-ratio").classList.add(`grid__image-ratio--${size}`), theme.settings.productImageCover || imageEl.classList.add("grid__image-contain"), addImageProperties(imageEl, imageSizes);
                const imageDiv = document.createElement("div");
                imageDiv.appendChild(clonedMarkup), output = imageDiv.innerHTML
            }

            function addImageProperties(imageEl, imageSizes2) {
                imageEl.src = product.image_responsive_url, imageEl.srcset = product.image_responsive_urls.toString(), imageEl.alt = product.title, imageEl.sizes = imageSizes2
            }
            return output
        };
        class AgeVerificationPopup extends HTMLElement {
            constructor() {
                super(), this.cookieName = this.id, this.cookie = Cookies.get(this.cookieName), this.classes = {
                ...
                }, this.declineButton = this.querySelector("[data-age-verification-popup-decline-button]"), this.declineContent = this.querySelector("[data-age-verification-popup-decline-content]"), this.content = this.querySelector("[data-age-verification-popup-content]"), this.returnButton = this.querySelector("[data-age-verification-popup-return-button]"), this.exitButton = this.querySelector("[data-age-verification-popup-exit-button]"), this.backgroundImage = this.querySelector("[data-background-image]"), this.mobileBackgroundImage = this.querySelector("[data-mobile-background-image]"), Shopify.designMode && (document.addEventListener("shopify:section:select", event => {
                    event.detail.sectionId === this.dataset.sectionId && this.init()
                }), document.addEventListener("shopify:section:load", event => {
                    if (event.detail.sectionId === this.dataset.sectionId) {
                        if (this.init(), this.dataset.testMode === "true" && this.cookie && Cookies.remove(this.cookieName), !sessionStorage.getItem(this.id)) return;
                        this.showDeclineContent()
                    }
                }), document.addEventListener("shopify:section:unload", event => {
                    event.detail.sectionId === this.dataset.sectionId && this.modal.close()
            ...
            init() {
                ...
                }), this.backgroundImage && (this.backgroundImage.style.display = "block"), theme.config.bpSmall && this.mobileBackgroundImage && (this.mobileBackgroundImage.style.display = "block"), this.modal.open(), theme.a11y.lockMobileScrolling(`#${this.id}`, document.querySelector("#MainContent")), this.declineButton && this.declineButton.addEventListener("click", e => {
                    e.preventDefault(), this.showDeclineContent(), Shopify.designMode && sessionStorage.setItem(this.id, "second-view")
                }), this.returnButton && this.returnButton.addEventListener("click", e => {
                    e.preventDefault(), this.hideDeclineContent();
                    const secondViewVisited = sessionStorage.getItem(this.id);
                    Shopify.designMode && secondViewVisited && sessionStorage.removeItem(this.id)
                }), this.exitButton && this.exitButton.addEventListener("click", e => {
                    e.preventDefault(), this.dataset.testMode === "false" && Cookies.set(this.cookieName, "entered", {
                        expires: 30,
                        sameSite: "none",
                        secure: !0
                    }), this.backgroundImage && (this.backgroundImage.style.display = "none"), theme.config.bpSmall && this.mobileBackgroundImage && (this.mobileBackgroundImage.style.display = "none"), this.modal.close(), theme.a11y.unlockMobileScrolling(`#${this.id}`, document.querySelector("#MainContent"))
                })
            }
            showDeclineContent() {
                this.declineContent.classList.remove(this.classes.inactiveDeclineContent), this.declineContent.classList.add(this.classes.activeDeclineContent), this.content.classList.add(this.classes.inactiveContent), this.content.classList.remove(this.classes.activeContent)
            }
            hideDeclineContent() {
                this.declineContent.classList.add(this.classes.inactiveDeclineContent), this.declineContent.classList.remove(this.classes.activeDeclineContent), this.content.classList.remove(this.classes.inactiveContent), this.content.classList.add(this.classes.activeContent)
            }
        }
        customElements.define("age-verification-popup", AgeVerificationPopup), theme.Maps = function() {
            var config = {
                    zoom: 14
                },
                apiStatus = null,
            ...
            window.gm_authFailure = function() {
                ...
                message.classList.add("map-section__error", "errors", "text-center"), message.innerHTML = error, document.querySelectorAll(selectors.mapOverlay).forEach(overlay => {
            ...
            function Map(container) {
                this.container = container, this.sectionId = this.container.getAttribute("data-section-id"), this.namespace = ".map-" + this.sectionId, this.map = container.querySelector(selectors.map), this.key = this.map.dataset.apiKey, errors = {
            ...
            function NewsletterPopup(container) {
                ...
                if (this.cookieName = "newsletter-" + sectionId, this.cookie = Cookies.get(this.cookieName), !!container && window.location.pathname !== "/challenge" && window.location.pathname !== "/password") {
                    this.data = {
                    ...
                    if (btn && btn.addEventListener("click", function() {
                        this.closePopup(!0)
                    }.bind(this)), (container.querySelector(".errors") || container.querySelector(".note--success")) && this.modal.open(), container.querySelector(".note--success")) {
                        this.closePopup(!0);
                        return
                    }
                    document.addEventListener("modalClose." + container.id, this.closePopup.bind(this)), this.cookie || this.initPopupDelay(), document.addEventListener("reminder:openNewsletter", () => {
                        this.modal.open()
                    })
                }
            }
            return NewsletterPopup.prototype = Object.assign({}, NewsletterPopup.prototype, {
                initPopupDelay: function() {
                    this.data.testMode !== "true" && setTimeout(function() {
                        if (sessionStorage.getItem("newsletterAppeared") === "true") {
                ...
                closePopup: function(success) {
                    ...
                    var expiry = success ? 200 : this.data.daysBeforeReappear,
                        ...
                        reminderAppeared = sessionStorage.getItem("reminderAppeared") === "true";
                    hasReminder && reminderAppeared ? Cookies.set(this.cookieName, "opened", {
                        path: "/",
                        expires: expiry
                    }) : hasReminder || Cookies.set(this.cookieName, "opened", {
                        path: "/",
                        expires: expiry
                    })
                },
            ...
            function Photoswipe(container, sectionId) {
                this.container = container, this.sectionId = sectionId, this.namespace = ".photoswipe-" + this.sectionId, this.gallery, this.images, this.items, this.inSlideshow = !1, !(!container || container.dataset.zoom === "false") && this.init()
            }
            return Photoswipe.prototype = Object.assign({}, Photoswipe.prototype, {
                ...
                triggerClick: function(evt) {
                    this.container.dataset && this.container.dataset.hasSlideshow === "true" ? this.inSlideshow = !0 : this.inSlideshow = !1, this.items = this.getImageData();
                ...
                preventiOS15Scrolling: function() {
                    ...
                    /iPhone|iPad|iPod/i.test(window.navigator.userAgent) && (this.syncHeight(), initialScrollPos = window.scrollY, document.documentElement.classList.add("pswp-open-in-ios"), window.addEventListener("resize", this.syncHeight), this.gallery.listen("destroy", () => {
        ...
        class ProductRecommendations extends HTMLElement {
            constructor() {
                super(), this.el = this, this.url = this.dataset.url, this.intent = this.dataset.intent, this.placeholder = this.querySelector(".product-recommendations-placeholder"), this.productResults = this.querySelector(".grid-product"), this.sectionId = this.dataset.sectionId, this.blockId = this.dataset.blockId, document.addEventListener("recommendations:loaded", e => {
                    this.colorImages = this.el.querySelectorAll(".grid-product__color-image"), this.colorImages.length && (this.swatches = this.el.querySelectorAll(".color-swatch--with-image"), this.colorSwatchHovering())
                })
            }
            connectedCallback() {
                fetch(this.url).then(response => response.text()).then(text => {
                    ...
                    this.placeholder.innerHTML = "", this.placeholder.innerHTML = recommendations.innerHTML, theme.reinitProductGridItem(this.el), this.slideshow = this.querySelector("[data-slideshow]"), this.slideshow && this.setupSlider(), document.dispatchEvent(new CustomEvent("recommendations:loaded", {
                        detail: {
                            section: this.el,
                            intent: this.intent
                        }
            ...
            setupSlider() {
                ...
                let prevNextButtons = !1,
                    pageDots = !0;
                controlType === "arrows" && (pageDots = !1, prevNextButtons = !0), perSlide < count && (this.flickity = new theme.Slideshow(this.slideshow, {
                    prevNextButtons,
                    pageDots,
                    adaptiveHeight: !0,
                    wrapAround: !1
            ...
            colorSwatchHovering() {
                this.swatches.forEach(swatch => {
                    ...
                    }.bind(this)), swatch.addEventListener("touchstart", function(evt) {
            ...
            setActiveColorImage(swatch) {
                ...
                imageEl.style.backgroundImage = "url(" + image + ")", imageEl.classList.add("is-active"), swatch.classList.add("is-active");
        ...
        customElements.define("product-recommendations", ProductRecommendations), theme.SlideshowSection = function() {
            var selectors = {
            ...
            return SlideshowSection.prototype = Object.assign({}, SlideshowSection.prototype, {
                init: function() {
                    ...
                    if (this.slideshow.classList.remove("loading", "loading--delayed"), this.slideshow.classList.add("loaded"), slides.length > 1) {
                        var sliderArgs = {
                ...
                onBlockSelect: function(evt) {
                    var slide = this.slideshow.querySelector(".slideshow__slide--" + evt.detail.blockId),
                        index = parseInt(slide.dataset.index);
                    this.flickity && typeof this.flickity.pause == "function" ? (this.flickity.goToSlide(index), this.flickity.pause()) : (this.initialIndex = index, setTimeout(function() {
            ...
            return StoreAvailability.prototype = Object.assign({}, StoreAvailability.prototype, {
                updateContent: function(variantId) {
                    ...
                    fetch(variantSectionUrl).then(function(response) {
                        ...
                        self2.container.innerHTML = html, self2.container.innerHTML = self2.container.firstElementChild.innerHTML, self2.container.querySelector(selectors.drawerOpenBtn) && (self2.drawer = new theme.Drawers("StoreAvailabilityDrawer", "availability")), self2.container.querySelector(selectors.modalOpenBtn) && (self2.modal = new theme.Modals("StoreAvailabilityModal", "availability"));
            ...
            function videoSection(container) {
                this.container = container, this.sectionId = container.getAttribute("data-section-id"), this.namespace = ".video-" + this.sectionId, this.videoObject, theme.initWhenVisible({
                    element: this.container,
                    callback: this.init.bind(this),
                    threshold: 500
                })
        ...
        class CountdownTimer extends HTMLElement {
            ...
            _calculate() {
                ...
                if (timeDifference > 0) {
                    ...
                    this.daysPlaceholder.innerHTML = intervals.days, this.hoursPlaceholder.innerHTML = intervals.hours, this.minutesPlaceholder.innerHTML = intervals.minutes, this.secondsPlaceholder.innerHTML = intervals.seconds, setTimeout(() => {
                        this.display.classList.add("countdown__display--loaded")
                    }, 1)
                } else this.completeMessage && this.messagePlaceholder && this.messagePlaceholder.classList.add("countdown__timer-message--visible"), this.hideTimerOnComplete === "true" && (this.display.classList.remove("countdown__display--visible"), this.display.classList.add("countdown__display--hidden")), !this.completeMessage && this.hideTimerOnComplete === "true" && this.block.classList.add("countdown__block--hidden"), this.timerComplete = !0
            }
        }
        customElements.define("countdown-timer", CountdownTimer);
        class HotSpots extends HTMLElement {
            constructor() {
                super(), this.el = this, this.buttons = this.querySelectorAll("[data-button]"), this.hotspotBlocks = this.querySelectorAll("[data-hotspot-block]"), this.blockContainer = this.querySelector("[data-block-container]"), this.colorImages = this.querySelectorAll(".grid-product__color-image"), this.colorSwatches = this.querySelectorAll(".color-swatch--with-image"), this._bindEvents(), this._setupQuickShop(), this.colorImages.length && this._colorSwatchHovering()
            }
            ...
            _showContent(id) {
                this.hotspotBlocks.forEach(block => {
                    block.dataset.hotspotBlock === id ? block.classList.add("is-active") : block.classList.remove("is-active")
                })
            }
            _setupQuickShop() {
                this.querySelectorAll('[data-block-type="product"]').length > 0 && (typeof theme.QuickShop == "function" ? new theme.QuickShop(this.blockContainer) : typeof theme.initQuickShop == "function" && theme.initQuickShop(), typeof theme.QuickAdd == "function" && new theme.QuickAdd(this.blockContainer))
            }
        }
        customElements.define("hot-spots", HotSpots);
        class ImageCompare extends HTMLElement {
            constructor() {
                super(), this.el = this, this.sectionId = this.dataset.sectionId, this.button = this.querySelector("[data-button]"), this.draggableContainer = this.querySelector("[data-draggable]"), this.primaryImage = this.querySelector("[data-primary-image]"), this.secondaryImage = this.querySelector("[data-secondary-image]"), this.calculateSizes(), this.active = !1, this.currentX = 0, this.initialX = 0, this.xOffset = 0, this.buttonOffset = this.button.offsetWidth / 2, this.el.addEventListener("touchstart", this.dragStart, !1), this.el.addEventListener("touchend", this.dragEnd, !1), this.el.addEventListener("touchmove", this.drag, !1), this.el.addEventListener("mousedown", this.dragStart, !1), this.el.addEventListener("mouseup", this.dragEnd, !1), this.el.addEventListener("mousemove", this.drag, !1), window.on("resize", theme.utils.debounce(250, () => {
                    this.calculateSizes(!0)
                })), document.addEventListener("shopify:section:load", event => {
                    event.detail.sectionId === this.sectionId && this.primaryImage !== null && this.calculateSizes()
                })
            }
            calculateSizes(hasResized = !1) {
                this.active = !1, this.currentX = 0, this.initialX = 0, this.xOffset = 0, this.buttonOffset = this.button.offsetWidth / 2, this.elWidth = this.el.offsetWidth, this.button.style.transform = `translate(-${this.buttonOffset}px, -50%)`, this.primaryImage && (this.primaryImage.style.width = `${this.elWidth}px`), hasResized && (this.draggableContainer.style.width = `${this.elWidth/2}px`)
            }
            dragStart(e) {
                e.type === "touchstart" ? this.initialX = e.touches[0].clientX - this.xOffset : this.initialX = e.clientX - this.xOffset, e.target === this.button && (this.active = !0)
            }
            dragEnd(e) {
                this.initialX = this.currentX, this.active = !1
            }
            drag(e) {
                this.active && (e.preventDefault(), e.type === "touchmove" ? this.currentX = e.touches[0].clientX - this.initialX : this.currentX = e.clientX - this.initialX, this.xOffset = this.currentX, this.setTranslate(this.currentX, this.button))
            }
            setTranslate(xPos, el) {
                ...
                newXpos < XposMin + boundaryPadding ? (newXpos = XposMin + boundaryPadding, newVal = boundaryPadding) : newXpos > XposMax - boundaryPadding && (newXpos = XposMax - boundaryPadding, newVal = this.elWidth - boundaryPadding), el.style.transform = `translate(${newXpos}px, -50%)`, this.draggableContainer.style.width = `${newVal}px`
            }
        }
        customElements.define("image-compare", ImageCompare), theme.Blog = function() {
            ...
            function CollectionHeader(container) {
                ...
                heroImageContainer ? (hasLoadedBefore && this.checkIfNeedReload(), heroImageContainer.classList.remove("loading", "loading--delayed"), heroImageContainer.classList.add("loaded")) : theme.settings.overlayHeader && theme.headerNav.disableOverlayHeader(), hasLoadedBefore = !0
            }
            ...
            return CollectionSidebar.prototype = Object.assign({}, CollectionSidebar.prototype, {
                init: function() {
                    this.container && (this.onUnload(), drawerStyle = this.container.dataset.style === "drawer", theme.FilterDrawer = new theme.Drawers("FilterDrawer", "collection-filters", !0))
                },
                forceReload: function() {
                    this.init()
                },
            ...
            function Collection(container) {
                this.container = container, this.sectionId = container.getAttribute("data-section-id"), this.namespace = ".collection-" + this.sectionId, this.sidebar = new theme.CollectionSidebar(container), this.context = container.getAttribute("data-context"), this.ajaxRenderer = new theme.AjaxRenderer({
                    sections: [{
            ...
            return Collection.prototype = Object.assign({}, Collection.prototype, {
                init: function() {
                    this.context && this.context === "featured-collection" ? this.colorSwatchHovering() : (this.initSort(), this.colorSwatchHovering(), this.initFilters(), this.initPriceRange(), this.sidebar.init())
                },
                initSort: function() {
                    this.sortSelect = document.querySelector(selectors.sortSelect), this.sortSelect && (this.defaultSort = this.getDefaultSortValue(), this.sortSelect.on("change" + this.namespace, this.onSortChange.bind(this)))
                },
                ...
                onSortChange: function() {
                    this.queryParams = new URLSearchParams(window.location.search), this.queryParams.set("sort_by", this.getSortValue()), this.queryParams.delete("page"), window.location.search = this.queryParams.toString()
                },
                colorSwatchHovering: function() {
                    ...
                    colorImages.length && this.container.querySelectorAll(selectors.colorSwatch).forEach(swatch => {
                        swatch.addEventListener("mouseenter", () => {
                            var id = swatch.dataset.variantId,
                                image = swatch.dataset.variantImage,
                                el = this.container.querySelector(".grid-product__color-image--" + id);
                ...
                initFilters: function() {
                    ...
                    tags.length && (this.bindBackButton(), theme.config.stickyHeader && (this.setFilterStickyPosition(), window.on("resize", theme.utils.debounce(500, this.setFilterStickyPosition))), document.querySelectorAll(selectors.activeTags).forEach(tag => {
                ...
                initPriceRange: function() {
                    document.querySelectorAll(selectors.priceRange).forEach(el => new theme.PriceRange(el, {
                        onChange: this.renderFromFormData.bind(this)
                    }))
                },
                tagClick: function(evt) {
                    var el = evt.currentTarget;
                    if (theme.FilterDrawer && theme.FilterDrawer.close(), el.classList.contains("no-ajax") || (evt.preventDefault(), isAnimating)) return;
                    isAnimating = !0;
                    const parent = el.parentNode,
                        newUrl = new URL(el.href);
                    this.renderActiveTag(parent, el), this.updateScroll(!0), this.startLoading(), this.renderCollectionPage(newUrl.searchParams)
                },
                onFormSubmit: function(evt) {
                    ...
                    this.renderActiveTag(parent, el), this.updateScroll(!0), this.startLoading(), this.renderFromFormData(formData)
                },
                ...
                renderActiveTag: function(parent, el) {
                    ...
                    parent.classList.contains(classes.activeTag) ? parent.classList.remove(classes.activeTag) : (parent.classList.add(classes.activeTag), el.closest("li").classList.contains(classes.removeTagParent) ? parent.remove() : document.querySelectorAll(selectors.activeTagList).forEach(list => {
                        const newTag = document.createElement("li"),
                            newTagLink = document.createElement("a");
                        newTag.classList.add("tag", "tag--remove"), newTagLink.classList.add("btn", "btn--small"), newTagLink.innerText = textEl.innerText, newTag.appendChild(newTagLink), list.appendChild(newTag)
                    }))
                },
                ...
                openCollapsible: function(el) {
                    el.classList.contains(classes.filterSidebar) && (el.style.height = "auto"), el.classList.add(classes.isOpen)
                },
                renderCollectionPage: function(searchParams, updateURLHash = !0) {
                    this.ajaxRenderer.renderPage(window.location.pathname, searchParams, updateURLHash).then(() => {
                        theme.sections.reinit("collection-grid"), this.updateScroll(!1), this.initPriceRange(), theme.reinitProductGridItem(), document.dispatchEvent(new CustomEvent("collection:reloaded")), isAnimating = !1
                    })
                },
                bindBackButton: function() {
                    window.off("popstate" + this.namespace), window.on("popstate" + this.namespace, function(state) {
                        if (state) {
                ...
                updateScroll: function(animate) {
                    ...
                    if (theme.config.bpSmall || (scrollTo -= 15), theme.config.stickyHeader) {
            ...
            function FooterSection(container) {
                this.container = container, this.localeDisclosure = null, this.currencyDisclosure = null, this.init()
            }
            ...
            return HeaderSection.prototype = Object.assign({}, HeaderSection.prototype, {
                init: function() {
                    ...
                    }, 500)), this.initDrawers(), this.initDisclosures(), theme.headerNav.init(), theme.announcementBar.init(), this.hoverMenu()
                },
                hoverMenu: function() {
                    ...
                    detailsEl.forEach(detail => {
                        ...
                        summary.addEventListener("click", e => {
                            ...
                            detail.hasAttribute("open") || (detailsEl.forEach(detail2 => {
                                detail2.removeAttribute("open"), detail2.setAttribute("aria-expanded", "false")
                            }), detail.setAttribute("open", ""), detail.setAttribute("aria-expanded", "true"))
                        }), detail.addEventListener("focusout", () => {
                            detail.hasAttribute("open") && !detail.matches(":focus-within") && (detail.removeAttribute("open"), detail.setAttribute("aria-expanded", "false"))
                        }), detail.addEventListener("mouseover", () => {
                            detail.hasAttribute("open") || (detail.setAttribute("open", ""), detail.setAttribute("aria-expanded", "true"))
                        }), detail.addEventListener("mouseleave", () => {
                            detail.hasAttribute("open") && (detail.removeAttribute("open"), detail.setAttribute("aria-expanded", "false"))
                        })
                    })
                },
                ...
                initDrawers: function() {
                    theme.NavDrawer = new theme.Drawers("NavDrawer", "nav"), theme.settings.cartType === "drawer" && (document.body.classList.contains("template-cart") || new theme.CartDrawer), theme.collapsibles.init(document.getElementById("NavDrawer"))
                },
                onUnload: function() {
            ...
            function Product(container) {
                ...
                this.inModal = container.dataset.modal === "true", this.modal, this.settings = {
                ...
                }, this.inModal && (this.settings.enableHistoryState = !1, this.settings.namespace = ".product-" + sectionId + "-modal", this.modal = document.getElementById("QuickShopModal-" + productId)), this.selectors = {
            ...
            return Product.prototype = Object.assign({}, Product.prototype, {
                init: function() {
                    this.inModal && (this.container.classList.add(classes.isModal), document.addEventListener("modalOpen.QuickShopModal-" + this.productId, this.openModalProduct.bind(this)), document.addEventListener("modalClose.QuickShopModal-" + this.productId, this.closeModalProduct.bind(this))), this.inModal || (this.formSetup(), this.productSetup(), this.videoSetup(), this.initProductSlider(), this.customMediaListners(), this.addIdToRecentlyViewed())
                },
                ...
                addIdToRecentlyViewed: function() {
                    ...
                    theme.recentlyViewed.recent.hasOwnProperty(handle) && delete theme.recentlyViewed.recent[handle], theme.recentlyViewed.recent[handle] = {
                        url,
                        aspectRatio,
                        featuredImage
                ...
                initVariants: function() {
                    ...
                    if (variantJson) {
                        ...
                        if (inventoryEl && (this.settings.inventory = !0, this.settings.inventoryThreshold = inventoryEl.dataset.threshold, this.updateInventory({
                            detail: {
                                variant: this.variants.currentVariant
                            }
                        }), this.container.on("variantChange" + this.settings.namespace, this.updateInventory.bind(this))), dynamicVariantsEnabled) {
                        ...
                        if (this.settings.imageSetName) {
                            ...
                            variantWrapper ? (this.settings.imageSetIndex = variantWrapper.dataset.index, this.container.on("variantChange" + this.settings.namespace, this.updateImageSet.bind(this))) : this.settings.imageSetName = null
                        }
                    }
                },
                    ...
                    if (cartBtn)
                        if (variant)
                            if (variant.available) {
                                cartBtn.classList.remove(classes.disabled), cartBtn.disabled = !1;
                                var defaultText = cartBtnText.dataset.defaultText;
                                cartBtnText.textContent = defaultText
                            } else cartBtn.classList.add(classes.disabled), cartBtn.disabled = !0, cartBtnText.textContent = theme.strings.soldOut;
                        else cartBtn.classList.add(classes.disabled), cartBtn.disabled = !0, cartBtnText.textContent = theme.strings.unavailable
                },
                updatePrice: function(evt) {
                    ...
                    if (variant)
                        if (this.cache.price.innerHTML = theme.Currency.formatMoney(variant.price, theme.settings.moneyFormat), variant.compare_at_price > variant.price) {
                            this.cache.comparePrice.innerHTML = theme.Currency.formatMoney(variant.compare_at_price, theme.settings.moneyFormat), this.cache.priceWrapper.classList.remove(classes.hidden), this.cache.price.classList.add(classes.onSale), this.cache.comparePriceA11y.setAttribute("aria-hidden", "false"), this.cache.priceA11y.setAttribute("aria-hidden", "false");
                            var savings = variant.compare_at_price - variant.price;
                            theme.settings.saveType == "percent" ? savings = Math.round(savings * 100 / variant.compare_at_price) + "%" : savings = theme.Currency.formatMoney(savings, theme.settings.moneyFormat), this.cache.savePrice.classList.remove(classes.hidden), this.cache.savePrice.innerHTML = theme.strings.savePrice.replace("[saved_amount]", savings)
                        } else this.cache.priceWrapper && this.cache.priceWrapper.classList.add(classes.hidden), this.cache.savePrice.classList.add(classes.hidden), this.cache.price.classList.remove(classes.onSale), this.cache.comparePriceA11y && this.cache.comparePriceA11y.setAttribute("aria-hidden", "true"), this.cache.priceA11y.setAttribute("aria-hidden", "true")
                },
                ...
                updateSku: function(evt) {
                    var variant = evt.detail.variant,
                        newSku = "";
                    variant && (variant.sku && (newSku = variant.sku), this.container.querySelector(this.selectors.sku).textContent = newSku)
                },
                updateInventory: function(evt) {
                    var variant = evt.detail.variant;
                    const inStockForOOSAndContinueSelling = !1;
                    if (!variant) {
                        this.toggleInventoryQuantity("hidden", !1), this.toggleIncomingInventory(!1);
                        return
                    }
                    if (!variant.inventory_management) {
                        this.toggleInventoryQuantity("visible", !1), this.toggleIncomingInventory(!1);
                        return
                    }
                    if (variant.inventory_management === "shopify" && window.inventories && window.inventories[this.productId]) {
                        ...
                        if (this.toggleInventoryQuantity(void 0, quantity), inStockForOOSAndContinueSelling && quantity <= 0 && policy === "continue") {
                            this.toggleInventoryQuantity("visible", !1), this.toggleIncomingInventory(!1);
                            return
                        }
                        incoming && !variant.available || quantity <= 0 && policy === "continue" ? this.toggleIncomingInventory(!0, next_incoming_date, policy) : this.toggleIncomingInventory(!1)
                    }
                },
                ...
                toggleIncomingInventory: function(showIncomingInventory, incomingInventoryDate, policy) {
                    const incomingInventoryEl = this.container.querySelector(this.selectors.incomingInventory),
                        incomingInventoryIcon = incomingInventoryEl.querySelector(".icon-and-text");
                ...
                videoSetup: function() {
                    ...
                    productVideos.forEach(vid => {
                        ...
                        type === "youtube" ? this.initYoutubeVideo(vid) : type === "vimeo" ? this.initVimeoVideo(vid) : type === "mp4" && this.initMp4Video(vid)
                    })
                },
                ...
                youtubePlayerReady: function(evt) {
                    ...
                    if (videoObjects[iframeId]) {
                        ...
                        obj.options.style !== "sound" && player.mute(), obj.parent.classList.remove("loading"), obj.parent.classList.add("loaded"), obj.parent.classList.add("video-interactable"), this._isFirstSlide(iframeId) && obj.options.style !== "sound" && player.playVideo()
                    }
                },
                ...
                stopVideos: function() {
                    for (var [id, vid] of Object.entries(videoObjects)) vid.videoPlayer ? typeof vid.videoPlayer.stopVideo == "function" && vid.videoPlayer.stopVideo() : vid.type === "mp4" && this.stopMp4Video(vid.id)
                },
                ...
                prepMediaOnSlide(slide) {
                    ...
                    nextMedia && (nextMedia.dispatchEvent(new CustomEvent("mediaVisible", {
                        bubbles: !0,
                        cancelable: !0
                    })), slide.querySelector(".shopify-model-viewer-ui__button").setAttribute("tabindex", 0), slide.querySelector(".product-single__close-media").setAttribute("tabindex", 0))
                },
                ...
                openModalProduct: function() {
                    ...
                    if (this.settings.modalInit) initialized = !0;
                        ...
                        fetch(url).then(function(response) {
                            ...
                            this.blocksHolder.innerHTML = "", this.blocksHolder.append(blocks), this.blocksHolder.classList.add("product-form-holder--loaded"), this.cacheElements(), this.formSetup(), this.updateModalProductInventory(), Shopify && Shopify.PaymentButton && Shopify.PaymentButton.init(), theme.collapsibles.init(this.container), document.dispatchEvent(new CustomEvent("quickview:loaded", {
                                detail: {
                                    productId: this.productId
                                }
                            }))
                        }.bind(this)), this.productSetup(), this.videoSetup(), this.settings.imageSetName ? this.variants ? this.initProductSlider() : document.addEventListener("quickview:loaded", function(evt) {
                            evt.detail.productId === this.productId && this.initProductSlider()
                ...
                updateModalProductInventory: function() {
                    window.inventories = window.inventories || {}, this.container.querySelectorAll(".js-product-inventory-data").forEach(el => {
                        var productId = el.dataset.productId;
                        window.inventories[productId] = {}, el.querySelectorAll(".js-variant-inventory-data").forEach(el2 => {
                ...
                customMediaListners: function() {
                    ...
                    modelViewers.length && modelViewers.forEach(el => {
                        el.addEventListener("shopify_model_viewer_ui_toggle_play", function(evt) {
                            this.mediaLoaded(evt)
                        }.bind(this)), el.addEventListener("shopify_model_viewer_ui_toggle_pause", function(evt) {
            ...
            return RecentlyViewed.prototype = Object.assign({}, RecentlyViewed.prototype, {
                init: function() {
                    if (!init) {
                        if (init = !0, Object.keys(theme.recentlyViewed.recent).length === 0 && theme.recentlyViewed.recent.constructor === Object) {
                        ...
                        }.bind(this)), Promise.all(promises).then(function(result) {
                ...
                setupOutput: function(products) {
                    ...
                    if (Object.keys(products).forEach(function(key) {
                        ...
                        if (product.handle === this.handle || typeof product.handle > "u") return;
                        i++;
                        const widths = [180, 360, 540, 720, 900];
                        product.url = theme.recentlyViewed.recent[product.handle] ? theme.recentlyViewed.recent[product.handle].url : product.url, product.image_responsive_url = theme.Images.buildImagePath(product.featured_image), product.image_responsive_urls = theme.Images.buildImagePath(product.featured_image, widths), product.image_aspect_ratio = theme.recentlyViewed.recent[product.handle].aspectRatio;
                    ...
                    }.bind(this)), allProducts.length === 0) {
                        this.container.classList.add("hide");
                        return
                    }
            ...
            function Testimonials(container) {
                ...
                var sectionId = container.getAttribute("data-section-id");
                this.slideshow = container.querySelector("#Testimonials-" + sectionId), this.namespace = ".testimonial-" + sectionId, this.slideshow && theme.initWhenVisible({
                    element: this.container,
                    callback: this.init.bind(this),
                    threshold: 600
                })
            ...
            return Testimonials.prototype = Object.assign({}, Testimonials.prototype, {
                init: function() {
                    this.slideshow.dataset.count <= 3 && (defaults.wrapAround = !1), this.flickity = new theme.Slideshow(this.slideshow, defaults), this.slideshow.dataset.count > 2 && (this.timeout = setTimeout(function() {
                ...
                onBlockSelect: function(evt) {
                    var slide = this.slideshow.querySelector(".testimonials-slide--" + evt.detail.blockId),
                        index = parseInt(slide.dataset.index);
                    clearTimeout(this.timeout), this.flickity && typeof this.flickity.pause == "function" && (this.flickity.goToSlide(index), this.flickity.pause())
                },
                onBlockDeselect: function() {
                    this.flickity && typeof this.flickity.play == "function" && this.flickity.play()
                }
            ...
            AOS && AOS.refreshHard(), document.documentElement.classList.contains("modal-open") || theme.initQuickShop(), theme.collapsibles.init()
...
//# sourceMappingURL=/cdn/shop/t/128/assets/theme.js.map
